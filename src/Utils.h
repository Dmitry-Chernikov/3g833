...
public:
    typedef union {
        typedef struct __attribute__ ((__packed__)) {
            uint8_t AddrModbus
            :
            1; // Адрес ведомого устройства
            uint8_t CodeCommand
            :
            3; // Код команды CMD
            uint16_t AddrParam
            :
            1; // Адрес параметра H и Адрес параметра L
            uint16_t DataParam
            :
            1; // Адрес параметра H и Адрес параметра L
            uint16_t CRC
 ...
 * @brief Данные о состоянии делятся на параметры контроля группы d, описание
 * неисправности привода переменного тока, состояние работы привода переменного
 * тока.
 */
enum ParameterGroup {
...
const ModbusFunction functionCodes[] = {
    {0x01, "Читать состояния дискретных выходов"}, {0x02, "Читать состояния дискретных входов"},
...
,
 к которому направлено сообщение
.
-
*
*
...
Данные(N байт) * *
 ...
 в зависимости от кода функции
.
Это может быть, например, адрес регистра и количество регистров для чтения
.
-
*
*
...
Пример пакета Modbus RTU для чтения
2
регистров : 
+
--
...
struct ModbusRegister {
    ...
    ModbusRegister(uint16_t addr, DataType t, uint16_t s, const std::string &desc) : address(addr), type(t), size(s),
        description(desc) {
    }
};
...
public:
    void addRegister(const ModbusRegister &reg) { registers.push_back(reg); }
...
int main() {
    ...
    if (reg) {
        std::cout << "Адрес: " << reg->address << ", Тип: " << static_cast<int>(reg->type) << ", Размер: " << reg->size
                << ", Описание: " << reg->description << std::endl;
    } else {
...
Serial
.
print (value); // Отправит значение в текстовом формате
Serial
.
println (value); // Отправит значение и добавит перевод строки

float floatValue = 25.5; // Пример значения
Serial
.
print(floatValue, 2); // Отправит значение с двумя знаками после запятой
Serial
.
println(floatValue, 2); // Отправит значение с переводом строки

uint8_t buffer[] = {0x01, 0x02, 0x03}; // Пример массива байтов
Serial
.
write(buffer, sizeof(buffer)); // Отправит массив байтов
