...
: Адрес устройства(от 1 до 247), к которому направлено сообщение.- **Код функции(1 байт) * * 
: Код функции, определяющий тип операции(например, чтение, запись и т.д.).- **Данные(N байт) * * 
: Данные, передаваемые в зависимости от кода функции.Это может быть, например, адрес регистра и количество регистров для чтения.- **CRC(2 байта) * *
