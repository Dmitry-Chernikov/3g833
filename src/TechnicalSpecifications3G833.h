...
namespace TechnicalSpecifications3G833 {
    ...
    constexpr double motorReturnsRPM = 930.00;
    // Максимальная скорость вращения мотора возвратно-поступательного движения
    ...
    namespace KinematicsReturns {
        ...
        constexpr double PULLEY_RATIO_1 = static_cast<double>(GEAR_PULLEY_SPEED_ONE1) / GEAR_PULLEY_SPEED_ONE2;
        // Первая передача 70/190 = 0.36
        constexpr double PULLEY_RATIO_2 = static_cast<double>(GEAR_PULLEY_SPEED_TWO1) / GEAR_PULLEY_SPEED_TWO2;
        // Вторая передача 84/175 = 0.48
        constexpr double PULLEY_RATIO_3 = static_cast<double>(GEAR_PULLEY_SPEED_THREE1) / GEAR_PULLEY_SPEED_THREE2;
        // Третья передача 110/155 = 0.71
        ...
        constexpr double SPEED_CONSTANT = (motorReturnsRPM * TRAVEL_PER_REVOLUTION_MM * GEARBOX_RATIO) / 1000.0;
        // (930*160.221*0.16) ≈ 23.840 м/мин
    ...
    namespace KinematicsRotational {
        ...
        // Коэффициенты редукции шкива от двигателя к входному валу
        constexpr double PULLEY_RATIO_1 = static_cast<double>(GEAR_PULLEY_SPEED_ONE1) / GEAR_PULLEY_SPEED_ONE2;
        // Первая передача 140/260 = 0.54
        constexpr double PULLEY_RATIO_2 = static_cast<double>(GEAR_PULLEY_SPEED_TWO1) / GEAR_PULLEY_SPEED_TWO2;
        // Вторая передача 200/215 = 0.93
        constexpr double PULLEY_RATIO_3 = static_cast<double>(GEAR_PULLEY_SPEED_THREE1) / GEAR_PULLEY_SPEED_THREE2;
        // Третья передача 235/180 = 1.30
        ...
        constexpr double CALC_RETURNS_SPEED_1 = motorSpindleRPM * GEARBOX_RATIO * PULLEY_RATIO_1;
        // (1430*0.217*0.54) ≈ 155.00 об/мин
        constexpr double CALC_RETURNS_SPEED_2 = motorSpindleRPM * GEARBOX_RATIO * PULLEY_RATIO_2;
        // (1430*0.217*0.93) ≈ 280.00 об/мин
        constexpr double CALC_RETURNS_SPEED_3 = motorSpindleRPM * GEARBOX_RATIO * PULLEY_RATIO_3;
        // (1430*0.217*1.30) ≈ 400.00 об/мин
        ...
        constexpr double calcRotationalPeripheralSpeed(const double diameter = 82.00, const byte speed = 1) {
            return speed == 1
                       ? PI * (diameter / 1000.0) * CALC_RETURNS_SPEED_1
                       : speed == 2
                             ? PI * (diameter / 1000.0) * CALC_RETURNS_SPEED_2
                             : speed == 3
                                   ? PI * (diameter / 1000.0) * CALC_RETURNS_SPEED_3
                                   : -1;
        }
    } // namespace KinematicsRotational
