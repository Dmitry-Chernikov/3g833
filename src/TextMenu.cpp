...
void updateErrorMessage(Adafruit_RGBLCDShield lcd, String mesLineOne, String mesLineTwo, uint8_t posLineOne,
                        uint8_t posLineTwo, DecIncrTypes typeOperation, float &paramManipulation, float maxParam,
...
bool hasReachedThreshold(float paramManipulation, float maxParam, float minParam, float threshold,
...
void changeParamMenu(DecIncrTypes typeOperation, float &paramManipulation, float maxParam, float minParam, bool *speeds,
                     StartLevelSpeed startSpeed, unsigned long &previousMillisSped,
                     const unsigned long *intervals, String messInc, String messDec, uint8_t posLineOne,
                     uint8_t posLineTwo) {
    ...
    const float thresholds[] = {
        (maxParam - minParam) * THRESHOLDS[0], (maxParam - minParam) * THRESHOLDS[1],
    ...
    if (isOutOfBounds(paramManipulation, maxParam, minParam)) {
        String mesLineTwo;
        updateErrorMessage(_lcd, "ERROR", mesLineTwo, posLineOne, posLineTwo, typeOperation, paramManipulation,
                           maxParam, minParam, messInc, messDec);
        return;
    }
...
void goBack() {
    // Функция обратного вызова, которая будет прикреплена к backLine. Выход в основное меню
...
void gotoLimitMenu() {
    // Процедура вызывает экран меню Лимиты
...
void gotoCylinderMenu() {
    // Процедура вызывает экран меню Цилиндр
...
void setLimitTop() {
    // Процедура копирует значение энкодера в значение верхнего лимита программного концевика
    if (!stateAutoCycleManual && stateStartFeed && !stateTopSlider) {
...
void increaseLimitTop() {
    // Процедура увеличивает значение верхнего лимита программного концевика
    changeParamMenu(DecIncrTypes::Inc, _data.limitTop, _data.limitBottom - smallestLength,
                    maxVerticalMovementSpindle - largestLength, _speeds, StartLevelSpeed::Speed_1, _previousMillisSped,
                    _intervals,
                    "TOP > ", "TOP < ", 5, 2);
}

void decreaseLimitTop() {
    // Процедура уменьшает значение верхнего лимита программного концевика
    changeParamMenu(DecIncrTypes::Dec, _data.limitTop, _data.limitBottom - smallestLength,
                    maxVerticalMovementSpindle - largestLength, _speeds, StartLevelSpeed::Speed_1, _previousMillisSped,
                    _intervals,
                    "TOP > ", "TOP < ", 5, 2);
}
...
void setLimitBottom() {
    // Процедура копирует значение энкодера в значение нижнего лимита программного концевика
    if (!stateAutoCycleManual && stateStartFeed && !stateTopSlider) {
...
void increaseLimitBottom() {
    // Процедура увеличивает значение нижнего лимита программного концевика
    changeParamMenu(DecIncrTypes::Inc, _data.limitBottom, maxVerticalMovementSpindle, _data.limitTop + smallestLength,
                    _speeds, StartLevelSpeed::Speed_1, _previousMillisSped, _intervals, "BOOTOM > ",
                    "BOTTOM < ", 5, 2);
}

void decreaseLimitBottom() {
    // Процедура уменьшает значение нижнего лимита программного концевика
    changeParamMenu(DecIncrTypes::Dec, _data.limitBottom, maxVerticalMovementSpindle, _data.limitTop + smallestLength,
                    _speeds, StartLevelSpeed::Speed_1, _previousMillisSped, _intervals, "BOOTOM > ",
                    "BOTTOM < ", 5, 2);
}
...
void modeEditValue() {
    ...
    if (((menuSystem.get_currentScreen() == &diameterScreen) && IncDecMode) || (
...
void increaseDiameter() {
    changeParamMenu(DecIncrTypes::Inc, _data.cylinderDiameter, permissibleDiameter, smallestDiameter, _speeds,
                    StartLevelSpeed::Speed_1, _previousMillisSped, _intervals, "Diametr > ", "Diametr < ", 5,
                    2);
}

void decreaseDiameter() {
    changeParamMenu(DecIncrTypes::Dec, _data.cylinderDiameter, permissibleDiameter, smallestDiameter, _speeds,
                    StartLevelSpeed::Speed_1, _previousMillisSped, _intervals, "Diametr > ", "Diametr < ", 5,
                    2);
}

void increaseAngle() {
    changeParamMenu(DecIncrTypes::Inc, _data.cylinderAngle, maximumScrubbingAngle, minimalScrubbingAngle, _speeds,
                    StartLevelSpeed::Speed_3, _previousMillisSped, _intervals, "Angle > ", "Angle < ", 5,
                    3);
}

void decreaseAngle() {
    changeParamMenu(DecIncrTypes::Dec, _data.cylinderAngle, maximumScrubbingAngle, minimalScrubbingAngle, _speeds,
                    StartLevelSpeed::Speed_3, _previousMillisSped, _intervals, "Angle > ", "Angle < ", 5,
                    3);
}
        ...
        if (buttons) {
            if (buttons & BUTTON_UP) {
                if (IncDecMode) {
                    ...
                    delay(500);
                    if (menuSystem.get_currentScreen() == &topScreen || menuSystem.get_currentScreen() == &bottomScreen
                        || menuSystem.get_currentScreen() == &diameterScreen ||
                        menuSystem.get_currentScreen() == &angleScreen || menuSystem.get_currentScreen() == &
                        oSecondaryScreen) {
                        ///
                    } else {
            ...
            if (!stateAutoCycleManual && stateStartFeed && !stateTopSlider) {
                ...
                while (_lcd.readButtons() > 0 && startMenu && !IncDecMode) {
                    startMenu = !stateMillisDelay(&previousMillisMenu, &intervalMenu);
                    lcdPrintString(String(second += 1), "", "", NOT_CHANGE_COLOR, NOT_CHANGE_COLOR, 0, 0, 1000, true,
                                   false);
                }
            }
        } else {
