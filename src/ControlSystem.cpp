...
void handleButtonStates() {
	// Общий стоп
	stateGeneralStop = triggerRS(stateGeneralStop, !digitalRead(buttonGeneralStop), !digitalRead(buttonStartFeed));
	// Подача-пуск
	stateStartFeed = triggerRS(stateStartFeed, !digitalRead(buttonStartFeed), !digitalRead(buttonGeneralStop));
	// Переключатель режимов: "Ввод хоны", "Ручной"
	stateAutoCycleManual = triggerRS(stateAutoCycleManual, digitalRead(switchAutoCycleManual),!digitalRead(switchAutoCycleManual));
	// Концевик парковки ползуна вверху исходного состояния
	stateTopSlider = triggerRS(stateTopSlider, digitalRead(switchTopSlider), !digitalRead(switchTopSlider));
	// Конец цикла
	stateEndCycle = triggerRS(stateEndCycle, !digitalRead(buttonEndCycle), stateGeneralStop);
}
...
void handleAutoCycleManual() {
	if (stateAutoCycleManual) {
		// Переключатель включен в режим Цикл
		handleAutoCycle();
	} else {
		// Переключатель включен в режим Ручной
		handleManualMode();
	}
}

void handleAutoCycle() {
	if (!digitalRead(electromagnetManual)) {
		// Если порт electromagnetManual включён
	...
	if (stateTopSlider) {
		// Если ползун на концевике парковки

		statePush = triggerRS(statePush, !digitalRead(buttonPush),digitalRead(buttonPush) || stateGeneralStop); // Толчковый ввод хоны
		...
		if (statePush && !_data.stateElectromagnetBottom) {
			// Если кнопка Толковая нажата и программный переключатель включён вверх
		...
		if (statePush && !digitalRead(endSwitchTop)) {
			// Если кнопка Толковая нажата и переключатель включён вверх
	...
	if (!stateTopSlider) {
		...
		if (stateEndCycle) {
			// Если кнопку Конец Цикла нажали
			...
			digitalWrite(electromagnetTop, _data.stateElectromagnetTop = false);
			// включить электромагнит движения вверх
		}

		if (!stateEndCycle) {
			// Если кнопку Конец Цикла не нажали

			if (stateSpindle) {
				// Если в ручном режиме ввели в цилиндр и запустили шпиндель и перевели переключатель в Цикл или просто Включён
			...
			if (!stateSpindle) {
				...
				statePush = triggerRS(statePush, !digitalRead(buttonPush), digitalRead(buttonPush) || stateSpindle || stateEndCycle || stateGeneralStop); // Толчковый ввод хоны

#ifdef ENABLE_PROGRAM_SWITCH
				if ((statePush && !_data.stateElectromagnetTop) || (statePush && !_data.stateElectromagnetBottom)) {
					// Если кнопка Толковая нажата и переключатель путевой включён вверх или вниз

					digitalWrite(electromagnetBrake, false); // включаем электромагнит растормаживания
					digitalWrite(electromagnetTop, _data.stateElectromagnetTop);
					// выключаем электромагнит движения вверх
					digitalWrite(electromagnetBottom, _data.stateElectromagnetBottom);
					// включаем электромагнит движения вниз
				}
				if ((!statePush && !_data.stateElectromagnetTop) || (!statePush && !_data.stateElectromagnetBottom)) {
					// Если кнопка Толковая не нажата и переключатель путевой включён вниз или верх
...
#ifdef ENABLE_SWITCH
				if (statePush && !digitalRead(endSwitchTop)) {
					// Если кнопка Толковая нажата и переключатель путевой включён вверх

					digitalWrite(electromagnetBrake, false); // включаем электромагнит растормаживания
					digitalWrite(electromagnetTop, true); // выключаем электромагнит движения вверх
					digitalWrite(electromagnetBottom, false); // включаем электромагнит движения вниз
				}
				if (statePush && !digitalRead(endSwitchBottom)) {
					// Если кнопка Толковая нажата и переключатель путевой включён вверх
				...
				if ((!statePush && !digitalRead(endSwitchTop)) || (!statePush && !digitalRead(endSwitchBottom))) {
					// Если кнопка Толковая не нажата и переключатель путевой включён вниз или вверх
...
void handleCycle() {
	...
	if (stateAutoCycleManual) {
		...
		if (stateEndCycle) {
			...
			if (stateTopSlider) {
				// Ползун поднялся в верх, исходное состояние конец цикла
		...
		if (!stateEndCycle && stateSpindle) {
			// Если кнопку Конец Цикла не нажали, Проверяем состояние шпинделя Если включен
			...
			if (!data.stateElectromagnetBottom) {
				// Если переключатель включен в положение верх

				digitalWrite(electromagnetTop, data.stateElectromagnetTop); // выключить электромагнит движения вверх
				delay(100);
				digitalWrite(electromagnetBottom, data.stateElectromagnetBottom);
				// включить электромагнит движения вниз
			}

			if (!data.stateElectromagnetTop) {
				// Если переключатель включен в положение низ

				digitalWrite(electromagnetBottom, data.stateElectromagnetBottom);
				// выключить электромагнит движения вниз
				delay(100);
...
#if defined(ENABLE_PROGRAM_SWITCH) && defined(ENABLE_SWITCH)
			if (!_data.stateElectromagnetBottom || !digitalRead(endSwitchTop)) {
				// Если переключатель включен в положение верх
			...
			if (!_data.stateElectromagnetTop || !digitalRead(endSwitchBottom)) {
				// Если переключатель включен в положение низ
		...
		if (!stateEndCycle && !stateSpindle) {
			// Если кнопку Конец Цикла не нажали, Проверяем состояние шпинделя Если выключен
	...
	if (!stateAutoCycleManual) {
		...
		// включить мотор шпинделя или отключить в зависимости от stateSpindle
		digitalWrite(motorSelfCoolant, !stateSpindle);
		// включить мотор помпы СОЖ или отключить в зависимости от stateSpindle

		if (!stateSpindle) {
			// Если шпиндель выключен, есть возможность запустить меню настройки
