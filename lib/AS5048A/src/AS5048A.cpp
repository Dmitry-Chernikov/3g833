...
// Programming control register. Programming must be enabled before burning memory. A check is mandatory before programming. See the programming procedure.
...
// (0-7) Automatic gain control value. 0 decimal represents a high magnetic field, 255 decimal represents a low magnetic field. (8-13) Diagnostic flags
const int AS5048A_MAGNITUDE = 0x3FFE; // CORDIC output power value
const int AS5048A_ANGLE = 0x3FFF;
// Angular output value, including zero position correction Resolution_ADC 14-bit resolution (0.0219°/LSB)
...
void AS5048A::absoluteAngleRotation(float *rotationAngle, float *angleCurrent, float *anglePrevious) {
    if (*angleCurrent != *anglePrevious) {
        ...
        if (*angleCurrent > *anglePrevious && ((*angleCurrent < 90) && (*anglePrevious > 270)) != true && (
                (*anglePrevious < 90) && (*angleCurrent > 270)) != true
        ...
        if (*anglePrevious > *angleCurrent && ((*angleCurrent < 90) && (*anglePrevious > 270)) != true && (
                (*anglePrevious < 90) && (*angleCurrent > 270)) != true
...
word AS5048A::calculateCircularMedian(word *adcValues, int sizeArray) {
    ...
    if (sizeArray > 1) {
        if ((sizeArray % 2) > 0) {
            sinSum = (sin(static_cast<double>(rotationRawToRadian(adcValues[(sizeArray / 2) - 1]))) + sin(
                          static_cast<double>(rotationRawToRadian(adcValues[sizeArray / 2]))) + sin(
                          static_cast<double>(rotationRawToRadian(adcValues[(sizeArray / 2) + 1])))) / 3;
            cosSum = (cos(static_cast<double>(rotationRawToRadian(adcValues[(sizeArray / 2) - 1]))) + cos(
                          static_cast<double>(rotationRawToRadian(adcValues[sizeArray / 2]))) + cos(
                          static_cast<double>(rotationRawToRadian(adcValues[(sizeArray / 2) + 1])))) / 3;
        } else {
            sinSum = (sin(static_cast<double>(rotationRawToRadian(adcValues[(sizeArray / 2) - 1]))) + sin(
                          static_cast<double>(rotationRawToRadian(adcValues[sizeArray / 2])))) / 2;
            cosSum = (cos(static_cast<double>(rotationRawToRadian(adcValues[(sizeArray / 2) - 1]))) + cos(
                          static_cast<double>(rotationRawToRadian(adcValues[sizeArray / 2])))) / 2;
        }
    } else {
...
word AS5048A::readAveragingMedian(word registerAddress, byte sizeValues, bool median) {
    ...
    if (median == true) {
        ...
        if (sizeValues > 1) {
            if ((sizeValues % 2) > 0) {
                readData = (adcValues[(sizeValues / 2) - 1] + adcValues[sizeValues / 2] + adcValues[
                                (sizeValues / 2) + 1]) / 3;
            } else {
